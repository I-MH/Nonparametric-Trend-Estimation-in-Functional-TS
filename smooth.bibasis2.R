...
smooth.bibasis2 <- function (sarg, targ, y, fdPars, fdPart, fdnames=NULL){
  ...
  #######
  #we add inprod of basis
  sinprod <- inprod(sbasis, sbasis)
  tinprod <- inprod(tbasis, tbasis)
  # meke it symmetric
  sinprod <- (sinprod + t(sinprod))/2
  tinprod <- (tinprod + t(tinprod))/2
  #######
  ...
  if (ns*nt > snbasis*tnbasis || lambdas > 0 || lambdat > 0) {
    ...
    if (lambdas > 0){
      ...
      Bmat <- Bmat + lambdas*kronecker(tinprod,penmats)
    }
    if (lambdat > 0) {
      ...
      Bmat <- Bmat + lambdat*kronecker(penmatt,sinprod)
    }
  ...
  smoothlist = list(bifdobj=bifdobj,  df=df,           gcv=gcv,
                    SSE=SSE,          y2cMap=y2cMap,   sarg=sarg,       
                    targ=targ,        y=y,             coef = coefmat)
